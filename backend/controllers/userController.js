...
export const followUnFollowUser = async (req,res) => {
    try {
        ...
        if(userToModify._id === req.user._id){
            return res.status(400).json({error : "You can't follow and unfollow by yourself"});
        }
...
export const updateUser = async (req ,res) => {
    try {
        ...
        if((!newPassword && currentPassword) || (!currentPassword && newPassword)){
            res.status(400).json({error : "Please provide both current and new passwords"})
        }
        if(currentPassword && newPassword){
            ...
            if(newPassword.length < 6){
                return res.status(400).json({error : "Passsword must have atleast 6 characters"})
            }
